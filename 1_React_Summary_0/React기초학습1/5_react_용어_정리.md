...
- React 엘리먼트에서 이벤트를 처리할 때는 일반적인 방식과는 구문상의 차이점이 있습니다.
  ...
  - React 이벤트 핸들러는 소문자가 아닌 캐멀케이스(camelCase)를 사용함.
  
  - JSX를 사용하면 문자열이 아닌 함수로 이벤트 핸들러를 전달함.

### 재조정

- 컴포넌트의 state나 props가 변경되면 React는 새로 반환된 컴포넌트를 이전에 렌더링된 컴포넌트와 비교하여 실제 DOM을 업데이트 해야하는지 결정 함.

- 두 컴포넌트가 동일하지 않다면, React는 DOM을 업데이트 함.

- 이 과정을 재조명(Reconciliation)이라고 함.
